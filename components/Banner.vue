<template>
    <div class="relative">
        <div class="flex flex-col gap-8 p-4 z-[10] relative bg-[#D6D3FF] rounded-[32px]">
          <Header />
      
          <div
            class="flex flex-col gap-4 items-center justify-center max-w-3xl mx-auto pb-[10rem] "
          >
            <h1
              class="text-3xl md:text-4xl font-semibold text-[#111111] lg:text-6xl text-center leading-10 lg:leading-[74px]"
            >
              Streamlining
              <span class="text-[#0047BA]">government communication</span> with
              <span class="text-[#0047BA]">twice</span> the
              <span class="text-[#0047BA]">speed</span> and
              <span class="text-[#0047BA]">security</span>
            </h1>
      
            <p class="text-lg font-normal leading-6 text-center">
              Enhancing governance and service delivery through digital transformation
              for sustainable progress through a secure and fast communication system.
            </p>
      
            <div class="flex flex-col sm:flex-row gap-3 items-center justify-center">
              <button class="btn-primary btn-primary-gradient">
                Go Paperless
                <svg
                  width="21"
                  height="20"
                  viewBox="0 0 21 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M13.879 9.25L9.856 5.227L10.9165 4.1665L16.75 10L10.9165 15.8335L9.856 14.773L13.879 10.75H4.75V9.25H13.879Z"
                    fill="currentColor"
                  />
                </svg>
              </button>
      
              <button class="btn-outline">
                  Learn More
                <svg
                  width="21"
                  height="20"
                  viewBox="0 0 21 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M13.879 9.25L9.856 5.227L10.9165 4.1665L16.75 10L10.9165 15.8335L9.856 14.773L13.879 10.75H4.75V9.25H13.879Z"
                    fill="currentColor"
                  />
                </svg>
              </button>
            </div>
          </div>
          
          <div class="h-[30px]"></div>
      
          <div class="h-[720px] bg-no-repeat bg-contain absolute w-full -bottom-[320px] mx-auto left-0 right-0 bg-center" style="background-image: url('/Group 1171274937.png');"></div>
          
          <div class="bg-contain bg-no-repeat size-[300px] absolute bottom-0" style="background-image: url('/img/v1.png');"></div>
          <div class="bg-contain bg-no-repeat size-[300px] absolute bottom-0 right-0" style="background-image: url('/img/v2.png');"></div>
        </div>
        
        <div class=" lg:flex sm/md:grid mt-[358px]  mx-8 ">
           <div class="max-w-[598px] left-0">
            <p class=" text-[16px] leading-5 font-normal text-[#111111] font-instrument-sans">
                Cabinet Memo ensures that you can securely send and receive data within the government ecosystem through a perfectly blended mixture of real-time and automated communication alongside a modern and secure system architecture 
            </p>
            <button class="btn-primary btn-primary-gradient mt-4">
                Get Started
                <svg
                  width="21"
                  height="20"
                  viewBox="0 0 21 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M13.879 9.25L9.856 5.227L10.9165 4.1665L16.75 10L10.9165 15.8335L9.856 14.773L13.879 10.75H4.75V9.25H13.879Z"
                    fill="currentColor"
                  />
                </svg>
              </button>
        </div> 
        <div >
              <div class="flex ">
                <img src="public/Rectangle 5702.svg" >
                <p class="font-medium text-[#7B13FD] ml-2 text-[16px]">
                What the platform can do
              </p>
            </div>
              <p class=" font-semibold text-2xl text-black  ">
                Enhancing the way inter-departmental communication is done in the government
              </p>

            </div>           
        </div>
        <!-- Grid containers -->
        <div class="flex  gap-[1rem] mx-8 absolute mt-[32px] ">
            <!-- NO. 1 -->
            <div class="grid left-0 top-0 rounded-xl bg-[#EBF3FE] ">
                <div class="bg-cover bg-no-repeat bg-[length: 100%_100%] h-[365px] mt-[37px] mr-[23px]" style="background-image: url('/collaboration tools.png');"></div>
            <p class="text-[18px] ml-[22px] mt-[20px] text-[#475467] ">
              <b class="font-semibold font-Intrumentsans text-black">Collaboration tools</b
              ><br />
              Seamless interagency collaboration in real-time to improve
              efficiency and productivity. Drafting, version control,
              commenting, and simultaneous editing within the defined Cabinet
              Memo workflow.
            </p>
          </div>
        <!-- NO. 2 -->
        <div class="grid left-0 top-0 rounded-xl bg-[#EBEFFF] relative">
                <div class="bg-cover bg-no-repeat bg-[length: 100%_100%] h-[365px] 
                 mt-[37px] ml-[23px]" style="background-image: url('/Digital signatures.png');"></div>
            <p class="text-[18px] ml-[22px] mt-[20px] text-[#475467] ">
              <b class="font-semibold font-Intrumentsans text-black ">Digital Signatures</b
              ><br />
              Electronic identity verification to ensure document integrity and preventing tampering. Digital signing is integrated within the workflows to reducing reliance on paper.
            </p>
          </div>
        <!-- NO. 3 -->
        <div class="grid left-0 top-0 rounded-xl bg-[#EBF3FE] relative">
                <div class="bg-cover bg-no-repeat bg-[length: 100%_100%] h-[365px]  mt-[37px]" style="background-image: url('/notifications.png');"></div>
            <p class="text-[18px] ml-[22px] mt-[40px] text-[#475467] ">
              <b class="font-semibold font-Intrumentsans text-black">Notifications</b
              ><br />
              Real-time notifications for instant updates and alerts, keeping users informed. These timely notifications enable prompt responses and facilitate immediate action.
            </p>
          </div>
        <!-- NO. 4 -->
        <div class="grid left-0 top-0 rounded-xl bg-[#F6EEFF] relative">
                <div class="bg-cover bg-no-repeat bg-[length: 100%_100%] h-[365px]  mt-[37px] mr-[23px]" style="background-image: url('/automatic workflows.png');"></div>
            <p class="text-[18px] ml-[22px] mt-[20px] text-[#475467] ">
              <b class="font-semibold font-Intrumentsans text-black">Automated Workflows</b
              ><br />
              Replacing manual processes with automated workflows to save time. Tasks and process between team members, real-time tracking to improve efficiency and accountability.
            </p>
          </div>

            </div>
            <div>
                
            </div>
        
       
    </div>
</template>

<script setup>
import Header from "@/components/Header.vue";
</script>
